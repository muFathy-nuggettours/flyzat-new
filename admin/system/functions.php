...
function getFirstFourWords($sentence = "")
{
    $words = explode(' ', $sentence);
    $firstFourWords = array_slice($words, 0, 4);
    return implode(' ', $firstFourWords);
}

function returnRequestStatusLabel($value)
{
    switch ($value) {
        case "0":
            return "<span class=\"label label-default label-block\">طلب جديد</span>";
            break;
        case "1":
            return "<span class=\"label label-primary label-block\">قيد التنفيذ</span>";
            break;
        case "2":
            return "<span class=\"label label-success label-block\">تم التنفيذ</span>";
            break;
    }
}
