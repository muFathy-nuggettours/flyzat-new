<? $search_data = ($value ? getData("system_database_airlines", "iata", $value) : null); ?>

<input type=hidden id=<?=$input?>-input name=<?=$input?> value="<?=$value?>" <? if ($mandatory){ ?>data-validation=requiredParentVisible data-validation-error-msg="لم تقم بتحديد خط الطيران"<? } ?>>
...
$("#<?=$input?>-select").select2({
	...
	placeholder: "ابحث عن خط الطيران بالكود او الإسم",
	...
	ajax: {
		...
		data: function (params){
			return { token:"<?=$token?>", action:"search_airlines", search:params.term, conditions:"<?=$conditions?>" };
		}
	}
});
